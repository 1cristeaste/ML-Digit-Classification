Evaluate <- function(true_label, pred_label) {
  ...
  correct = sum(true_label == pred_label)
  error <- (length(true_label) - correct)/length(true_label)
...
Predict_logis <- function(data_feature, beta, beta0, type) {
  ...
  prob = exp(pred_vec)/(1+exp(pred_vec))
...
Comp_gradient <- function(data_feature, data_label, beta, beta0, lbd) {
  ...
  # TODO:                                                             #
  #####################################################################
  prob = Predict_logis(data_feature, beta, beta0, "prob")
  
  
  
  beta0 = (1/n) *sum(-data_label + prob)
  beta_other = (1/n) * (colSums(as.numeric(prob - data_label) * data_feature)) + lbd * beta
  grad = c(beta0, beta_other)
...
Comp_loss <- function(data_feature, data_label, beta, beta0, lbd) {
  ...
  # TODO:                                                             #
  #####################################################################
  prob = prob = Predict_logis(data_feature, beta, beta0, "prob")
  n = length(data_label)
  
  loss = (-1/n)*sum(data_label*log(prob)+(1-data_label)*log(1-prob))+(lbd/2)*norm(beta, type = "2")^2
...
Penalized_Logistic_Reg <- function(x_train, y_train, lbd, stepsize, max_iter) {
  ...
  #####################################################################
  gradient = Comp_gradient(x_train, y_train, beta_cur, beta0_cur, lbd)
  
  for (i in 1:max_iter){
    # gradient = gradient - stepsize*gradient
    
    gradient = Comp_gradient(x_train, y_train, beta_cur, beta0_cur, lbd)
    beta_new = beta_cur - stepsize * gradient[-1]
    beta0_new = beta0_cur - stepsize * gradient[1]
    # Comp_gradient(x_train, y_train, beta_cur, beta0_cur, lbd)
    
    predicted = Predict_logis(x_train, beta_new, beta0_new, "class")
    
    loss_vec[i] = Comp_loss(x_train, y_train, beta_new, beta0_new, lbd)
    error_vec[i] = Evaluate(y_train, predicted)
